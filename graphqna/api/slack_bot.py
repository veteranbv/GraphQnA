...
logger.info(
    f"SLACK_SIGNING_SECRET exists: {bool(os.environ.get('SLACK_SIGNING_SECRET'))}"
)
...
MONITORED_CHANNELS = os.environ.get(
    "SLACK_MONITORED_CHANNELS", "general,ask_vivun"
).split(",")
ALWAYS_RESPOND_CHANNELS = os.environ.get(
    "SLACK_ALWAYS_RESPOND_CHANNELS", "ask_vivun"
).split(",")
...
def query_graphqna(question: str, method: str = "graphrag") -> Dict[str, Any]:
    ...
    try:
        ...
        payload = {"query": question, "retrieval_method": method, "top_k": 5}

        # Add API key to headers if available
        headers = {"Content-Type": "application/json"}
        api_key = os.environ.get("GRAPHQNA_API_KEY")
        if api_key:
            headers["X-API-Key"] = api_key

        logger.info(f"Querying GraphQnA API at {url}")
        response = requests.post(url, json=payload, headers=headers)

        # Check for error response
        if response.status_code != 200:
            logger.error(
                f"Error querying GraphQnA API: {response.status_code} - {response.text}"
            )
            return {
                "answer": f"Sorry, I couldn't process your question. (Error: {response.status_code})",
                "query": question,
                "context": [],
                "query_time": 0,
            }
        ...
        logger.error(f"Exception querying GraphQnA API: {str(e)}")
        return {
...
def handle_app_mention(event, say, client):
    ...
    if not question:
        try:
            ...
            say(
                "Please ask me a question! For example: @GraphQnA What is a knowledge graph?"
            )
        return
...
def handle_channel_message(event, say, client):
    ...
    try:
        ...
        is_always_respond = channel_name_lower in [
            c.strip().lower() for c in ALWAYS_RESPOND_CHANNELS
        ]
        logger.info(
            f"Processing message in channel: {channel_name} (always respond: {is_always_respond})"
        )
    except Exception as e:
    ...
    has_good_answer = not any(
        phrase.lower() in response.get("answer", "").lower()
    ...
    if not has_good_answer and not is_always_respond:
        logger.info(
            "Not responding as answer indicates insufficient information and not in always-respond channel"
        )
        return
    ...
    if not has_good_answer and is_always_respond:
        logger.info(
            "In always-respond channel but no good answer - will add disclaimer"
        )
        current_answer = response.get("answer", "")
        response["answer"] = (
            "I'm not entirely sure about this, but here's what I found:\n\n"
            + current_answer
        )
    ...
    try:
        ...
        logger.info("Sent response in thread")
    except Exception as e:
...
def handle_feedback_submission(ack, body, view, say, client):
    ...
    try:
        client.chat_postMessage(
            channel=user,
            text="Thank you for your feedback! We'll use it to improve future responses.",
        )
    except Exception as e:
...
def start_slack_bot():
    ...
    try:
        ...
        if not os.environ.get("SLACK_BOT_TOKEN") or not os.environ.get(
            "SLACK_APP_TOKEN"
        ):
            logger.error(
                "Missing required environment variables: SLACK_BOT_TOKEN or SLACK_APP_TOKEN"
            )
            print(
                "Error: Please set SLACK_BOT_TOKEN and SLACK_APP_TOKEN environment variables"
            )
            print("Example:")
        ...
        if bot_token:
            masked_token = (
                f"{bot_token[:10]}...{bot_token[-4:]}" if len(bot_token) > 14 else "***"
            )
        ...
        if app_token:
            masked_token = (
                f"{app_token[:10]}...{app_token[-4:]}" if len(app_token) > 14 else "***"
            )
        ...
        print(
            f"  - Regular messages in monitored channels: {', '.join(MONITORED_CHANNELS)}"
        )
        print("\nResponse behavior:")
        print("  - All channel responses will be in threads")
        print("  - In regular monitored channels: only respond when confident")
        print(
            "  - In always-respond channels: respond to all messages, even with low confidence"
        )
